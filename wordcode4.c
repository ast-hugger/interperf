/*
    Same as wordcode3.c, except:

        - Opcode dispatch by indexing into a function pointer array instead of a switch.

    Observations (Clang):

        - Gained back the 20-25% performance hit of wordcode3
        - Handling RET the uniform way is more of a challenge; not doing that
 */
...
typedef void (*instr_handler)(struct interpreter *interp);

static instr_handler ops[] = {
    execute_lit,
    execute_load,
    execute_call,
    execute_prim,
...
static word_t execute(struct interpreter *interp) {
    while (true) {
        ...
        if (opcode == RET) return pop(interp);
        ops[opcode](interp);
    }
}
