/*
    Derived from comboinstructions.c:

    Introduce an additional SUB2 instruction, used instead of a sequence of two SUB1 instructions.

    Results:

        - 4% performance improvement compared to comboinstructions
        - 3.74x performence improvement compared to wordcode2
...
static word_t execute(word_t arg) {
    ...
    static word_t fib_code[] = {
        ...
        /*  5 */  OP(JT), 18, // JT 23 = 5 + 18
        /*  7 */  OP(LOAD), -1, // arg
        /*  9 */  OP(SUB1),
        /* 10 */  OP(CALL), 0, 1, // fib, 1 arg
        /* 13 */  OP(LOAD), -1, // arg
        /* 15 */  OP(SUB2),
        /* 16 */  OP(CALL), 0, 1, // fib, 1 arg
        /* 19 */  OP(PRIM), 2, // add
        /* 21 */  OP(JMP), 3, // JMP 25 = 22 + 3
        /* 23 */  OP(CONST_1),
        /* 24 */  OP(RET)
    };
...
SUB1: // the original combo instruction
    *((int64_t *)(sp - 1)) -= 1;
    GOTO_NEXT;

SUB2: // and an additional one
    *((int64_t *)(sp - 1)) -= 2;
    GOTO_NEXT;
